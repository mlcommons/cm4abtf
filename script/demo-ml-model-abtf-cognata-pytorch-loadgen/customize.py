...
def preprocess(i):
    ...
    if env.get('CM_MLPERF_LOADGEN_QUERY_COUNT','') != '' and not env.get('CM_TMP_IGNORE_MLPERF_QUERY_COUNT', False) and env.get('CM_MLPERF_RUN_STYLE','') != "valid":
...
#    cmd, run_dir = get_run_cmd(os_info, env, scenario_extra_options, mode_extra_options, dataset_options, mlperf_implementation)
    cmd, run_dir = get_run_cmd_reference(os_info, env, scenario_extra_options, mode_extra_options, dataset_options, script_path)
...
#        cognata_dataset_path = env['CM_DATASET_PATH'] # Using open images dataset for some tests
        ...
        cmd = env['CM_PYTHON_BIN_WITH_PATH'] + f" python{os.path.sep}main.py " + " --profile "+env['CM_MODEL']+"-"+env['CM_MLPERF_BACKEND'] + \
